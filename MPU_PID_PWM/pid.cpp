...
void controladorPID::reiniciar_integrador(){
    integral = 0.0;
}
float controladorPID::computar(float setpoint, float current){
    ...
    if(error < 80.0 || error > -80.0){
        
        
    if(error < 3.0 && error > -3.0){
        integral = integral + (error * ki); // Ts = 3ms (tiempo de muestreo)
    }
    // integral = 0.0;
    ...
    */
        ...
        float output = kp*error + integral + (kd*(error-prevError)/0.003);
        prevError = error;
        return output;
    }else{
        return 0.0;
